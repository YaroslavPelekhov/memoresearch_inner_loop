...
read only `probability_calibration`.
...
Activation is a separate, explicit protocol decision.

## Locked-test decision

The overall cascade is the primary analysis; per-rung results are secondary
diagnostics. After model, intervals, rungs, and gates are frozen, run the policy
once on `locked_test` and create one outcome per candidate:
...
python tools/evaluate-multifidelity-probe.py \
  --predictions runs/statistics/locked-test-probe-predictions.jsonl \
