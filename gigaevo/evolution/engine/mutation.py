...
async def generate_mutations(
    ...
    *,
    ...
    try:
        history_setter = getattr(parent_selector, "set_population_history", None)
        if callable(history_setter):
            try:
                history_setter(
                    await storage.snapshot.get_all(
                        storage, exclude=EXCLUDE_STAGE_RESULTS
                    )
                )
            except Exception as exc:
                logger.warning(
                    "[mutation] Failed to provide population history to parent selector: {}",
                    exc,
                )
