...
class TargetFitnessStopper(EvolutionStopper):
    def __init__(self, target_fitness: float = 1.0, epsilon: float = 1e-9) -> None:
    ...
    def should_stop(self, ctx: StopContext) -> StopDecision:
        ...
        if ctx.best_fitness + self.epsilon >= self.target_fitness:
            return StopDecision(
                stop=True,
                reason=(
                    f"Reached target_fitness={self.target_fitness:g} "
                    f"(best_fitness={ctx.best_fitness:g})"
                ),
            )
        return StopDecision(stop=False, reason="")
