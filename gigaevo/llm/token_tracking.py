...
class TokenUsage(BaseModel):
    ...
    context: int = 0
    cached_context: int = 0  # Subset of context tokens
    generated: int = 0
    ...
    def from_response(cls, response: Any) -> "TokenUsage | None":
        ...
        detailed = response.response_metadata.get("codex_usage") or {}
        ...
        if details := usage.get("prompt_tokens_details"):
            cached_context = details.get("cached_tokens", 0) or 0
        ...
        return cls(
            ...
            estimated_cost_usd=detailed.get("estimated_cost_usd") or 0.0,
        )
...
class TokenTracker(BaseModel):
    ...
    def track(self, response: Any, model_name: str) -> None:
        ...
        with self.lock:
            ...
            cum.cached_context += usage.cached_context
            cum.generated += usage.generated
            cum.reasoning += usage.reasoning
            cum.total += usage.total
            cum.estimated_cost_usd += usage.estimated_cost_usd
    ...
    def _write_metrics(
        ...
        self.writer.scalar(
            "cached_context_tokens", float(usage.cached_context), path=path
        )
        ...
        self.writer.scalar("estimated_cost_usd", usage.estimated_cost_usd, path=path)

        self.writer.scalar(
            "cumulative_context_tokens", float(cumulative.context), path=path
        )
        self.writer.scalar(
            "cumulative_cached_context_tokens",
            float(cumulative.cached_context),
            path=path,
        )
        self.writer.scalar(
