...
def normalize_memory_card(
    ...
    if category == "program" or program_id:
        return ProgramCard(
            ...
            parent_program_ids=_to_list(raw.get("parent_program_ids")),
            generation=_to_int(raw.get("generation"), default=0),
            metrics={
                str(key): float(value)
            ...
            metric_deltas={
                str(key): float(value)
                for key, value in (raw.get("metric_deltas") or {}).items()
                if isinstance(value, (int, float))
            }
            if isinstance(raw.get("metric_deltas"), dict)
            else {},
            verdict=str(raw.get("verdict") or ""),
            commit=str(raw.get("commit") or ""),
            parent_commit=str(raw.get("parent_commit") or ""),
            changed_files=_to_list(raw.get("changed_files")),
            reflection=(
                dict(raw.get("reflection") or {})
                if isinstance(raw.get("reflection"), dict)
                else {}
            ),
            artifact_refs=(
                dict(raw.get("artifact_refs") or {})
                if isinstance(raw.get("artifact_refs"), dict)
                else {}
            ),
            duration_seconds=to_float(raw.get("duration_seconds"), default=None),
            approved_idea_id=str(raw.get("approved_idea_id") or ""),
