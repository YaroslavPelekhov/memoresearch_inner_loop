...
class RepoSeedLoader:
    """Seed evolution from a Git repository snapshot instead of Python files.

    The created Program.code is a RepoCandidateManifest JSON pointer. The real
    candidate artifact remains in Git.
    """
    ...
    def __init__(
        ...
        *,
        ...
        initial_commit_message: str = "gigaevo initial repo seed",
        name: str = "repo-seed",
    ):
        self.source_repo = Path(source_repo)
        self.ref = ref
        self.refs = tuple(refs) if refs else (ref,)
        if len(set(self.refs)) != len(self.refs):
            raise ValueError("Repo seed refs must be unique")
        self.entrypoint = entrypoint
        self.init_if_needed = init_if_needed
        self.create_initial_commit = create_initial_commit
        self.initial_commit_message = initial_commit_message
        self.name = name

    async def load(self, storage: RedisProgramStorage) -> list[Program]:
        repo = ensure_git_repo(self.source_repo, init_if_needed=self.init_if_needed)
        if self.create_initial_commit:
            ensure_initial_commit(repo, message=self.initial_commit_message)

        programs: list[Program] = []
        dirty_at_seed = bool(status_porcelain(repo).strip())
        for ref in self.refs:
            commit = rev_parse(repo, ref)
            branch = current_branch(repo) if len(self.refs) == 1 else ref
            manifest = RepoCandidateManifest(
                ...
                extra={
                    "seed_ref": ref,
                    "dirty_at_seed": dirty_at_seed,
                },
            )
            program_name = self.name if len(self.refs) == 1 else f"{self.name}:{ref}"
            program = Program(code=manifest.to_program_code(), name=program_name)
            program.metadata = {
                "source": "repo_seed",
                "strategy_name": ref,
                "seed_ref": ref,
                ...
                "repo_entrypoint": self.entrypoint,
            }
            await storage.add(program)
            programs.append(program)
        return programs
