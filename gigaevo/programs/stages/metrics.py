...
class EnsureMetricsStage(Stage):
    ...
    async def compute(self, program: Program) -> StageIO:
        ...
        # A technical failure can only report the measurements it reached.  Keep
        # those real values and fill unavailable scientific metrics with their
        # configured sentinels instead of replacing the whole result or raising
        # a misleading "missing metric" error.
        metrics_input = dict(sentinel_metrics)
        if params.candidate is not None:
            metrics_input.update(params.candidate.data)
