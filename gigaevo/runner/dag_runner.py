...
class DagRunner:
    ...
    async def _launch(self) -> None:
        ...
        prepared: list[tuple[Program, DAG]] = []
        ...
        for program in candidates:
            ...
            prepared.append((program, dag))

        if not prepared:
            return

        # Persist QUEUED → RUNNING before creating tasks.  Refresh DAGs can be
        # entirely cached and finish in the same event-loop turn; creating tasks
        # first allowed their RUNNING → DONE write to race with (and be
        # overwritten by) this batch transition, leaving false RUNNING orphans.
        prepared_ids = [program.id for program, _ in prepared]
        try:
            ...
            for _, dag in prepared:
                with contextlib.suppress(Exception):
                    dag.teardown()
            return

        running_ids = set(prepared_ids)
        if count != len(prepared_ids):
            # The storage API returns only a count.  Re-read after a partial
            # transition so that no DAG executes without owning RUNNING state.
            logger.warning(
                "[DagScheduler] batch mark-started transitioned {} of {} programs; "
                "reconciling before launch",
                count,
                len(prepared_ids),
            )
            try:
                persisted = await self._storage.mget(prepared_ids)
                running_ids = {
                    program.id
                    for program in persisted
                    if program is not None and program.state == ProgramState.RUNNING
                }
            except Exception as e:
                logger.error(
                    "[DagScheduler] could not reconcile partial launch transition: {}",
                    e,
                )
                running_ids = set()

        launched_count = 0
        for program, dag in prepared:
            if program.id not in running_ids:
                with contextlib.suppress(Exception):
                    dag.teardown()
                continue
        ...
        self._metrics.dag_runs_started += launched_count
        logger.info("[DagScheduler] launched {} programs", launched_count)
