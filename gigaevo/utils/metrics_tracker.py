...
class MetricsTracker:
    ...
    def _is_observed_metric(self, key: str, value: object) -> bool:
        if not isinstance(value, int | float):
            return False
        spec = self._ctx.specs.get(key)
        return spec is None or not spec.is_sentinel(float(value))
                ...
                try:
                    if await self._process_program(prog):
                        ...
                        metrics_hash = tuple(
                            ...
                            if k != VALIDITY_KEY and self._is_observed_metric(k, v)
                        )
    ...
    async def _process_program(self, program: Program) -> bool:
        ...
        numeric_metrics = {
            ...
            if k != VALIDITY_KEY and self._is_observed_metric(k, val)
        }
        ...
        for key, val in metrics.items():
            if key == VALIDITY_KEY or not self._is_observed_metric(key, val):
                continue
            fval = float(val)
