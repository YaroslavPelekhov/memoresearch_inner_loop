...
from gigaevo.repo_harness.auto_seed import configure_repo_harness_auto_seed
...
async def run_experiment(cfg: DictConfig) -> None:
    ...
    auto_seed = configure_repo_harness_auto_seed(cfg, repo_root=repo_root)
    if auto_seed is not None:
        action = "Created" if auto_seed.created else "Reusing"
        logger.info(
            "{} repo-harness auto seed at {} (spec={}, variants={}, refs={})",
            action,
            auto_seed.source_repo,
            auto_seed.spec_path,
            auto_seed.variants or (auto_seed.variant,),
            auto_seed.refs,
        )
    ...
    try:
        ...
        if has_data and resume:
            ...
            await evolution_engine.strategy.reset_state()
