...
from gigaevo.evolution.engine.core import EvolutionEngine, EvolutionStopRequested
...
# ---------------------------------------------------------------------------
# _format_archive_best_summary
# ---------------------------------------------------------------------------


class TestArchiveBestSummary:
    async def test_reads_current_archive_instead_of_tracker_cache(self) -> None:
        """Generation summaries use current archive metrics, not stale tracker state."""
        engine = _make_engine()
        engine._metrics_tracker.metrics_context = _metrics_context()
        engine._metrics_tracker.format_best_summary.return_value = (
            " best=[fitness=0.6667]"
        )

        seed = _prog()
        seed.metrics = {"is_valid": 1.0, "fitness": 0.666667, "system_errors": 0.0}
        solved = _prog()
        solved.metrics = {"is_valid": 1.0, "fitness": 1.0, "system_errors": 1.0}

        engine.strategy.get_program_ids.return_value = [seed.id, solved.id]
        engine.storage.mget.return_value = [seed, solved]

        summary = await engine._format_archive_best_summary()

        assert "fitness=1.0000" in summary
        assert "system_errors=0" in summary
        assert "fitness=0.6667" not in summary
        assert engine._build_stop_context().best_fitness == pytest.approx(1.0)
...
class TestCreateMutants:
    ...
    async def test_three_empty_mutation_generations_request_stop(self) -> None:
        engine = _make_engine()
        elites = [_prog()]
        ...
        with patch(
            ...
            assert await engine._create_mutants(elites) == []
            assert await engine._create_mutants(elites) == []
            with pytest.raises(
                EvolutionStopRequested,
                match="no persisted candidates for 3 consecutive generations",
            ):
                await engine._create_mutants(elites)

        assert mock_gen.await_count == 3

    async def test_successful_mutation_resets_empty_generation_counter(self) -> None:
        engine = _make_engine()
        elites = [_prog()]
        ...
        with patch(
            ...
            assert await engine._create_mutants(elites) == []
            assert await engine._create_mutants(elites) == ["mut-1"]
            assert await engine._create_mutants(elites) == []
            assert await engine._create_mutants(elites) == []
...
class TestRunLoop:
    async def test_terminal_hook_condition_stops_without_retrying(self) -> None:
        engine = _make_engine()
        engine.config.stopper = EvolutionStopper()
        engine.step = AsyncMock(
            side_effect=EvolutionStopRequested("no valid scientific baseline")
        )

        await asyncio.wait_for(engine.run(), timeout=ENGINE_TEST_TIMEOUT)

        engine.step.assert_awaited_once()
        engine.storage.save_run_state.assert_any_await(
            "engine:completion_reason", "no valid scientific baseline"
        )
