...
class TestTargetFitnessStopper:
    def test_does_not_stop_before_target(self) -> None:
        stopper = TargetFitnessStopper(target_fitness=1.0)
        assert stopper.should_stop(_ctx(best_fitness=0.999)).stop is False

    def test_stops_at_target(self) -> None:
        stopper = TargetFitnessStopper(target_fitness=1.0)
        result = stopper.should_stop(_ctx(best_fitness=1.0))
        assert result.stop is True
        assert "target_fitness=1" in result.reason

    def test_none_fitness_does_not_stop(self) -> None:
        stopper = TargetFitnessStopper(target_fitness=1.0)
        assert stopper.should_stop(_ctx(best_fitness=None)).stop is False
...
class TestHydraInstantiation:
    ...
    def test_target_fitness_yaml(self) -> None:
        from hydra.utils import instantiate
        from omegaconf import OmegaConf

        stopper_cfg = OmegaConf.load("config/stopper/target_fitness.yaml")
        cfg = OmegaConf.create({"target_fitness": 0.95, "stopper": stopper_cfg})
        stopper = instantiate(cfg.stopper)
        assert isinstance(stopper, TargetFitnessStopper)
        assert stopper.target_fitness == pytest.approx(0.95)
    ...
    def test_composite_target_fitness_yaml(self) -> None:
        from hydra.utils import instantiate
        from omegaconf import OmegaConf

        stopper_cfg = OmegaConf.load(
            "config/stopper/max_generations_or_target_fitness.yaml"
        )
        ...
        assert len(stopper.children) == 2
        assert isinstance(stopper.children[0], MaxGenerationsStopper)
        assert isinstance(stopper.children[1], TargetFitnessStopper)
