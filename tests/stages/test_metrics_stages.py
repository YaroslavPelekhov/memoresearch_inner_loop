...
class TestEnsureMetricsStage:
    ...
    async def test_missing_required_key_uses_sentinel(self):
        """A partial technical result preserves values and fills missing metrics."""
        stage = _make_ensure_stage()
        ...
        assert result.status == StageState.COMPLETED
        assert program.metrics["cost"] == 10.0
        assert program.metrics["score"] == -1.0
