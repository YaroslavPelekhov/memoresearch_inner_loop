...
@pytest.mark.parametrize("variant", _group_choices("parent_selector"))
def test_parent_selector_variant_composes(variant: str):
    cfg = _compose(
        "experiment=repo_harness",
        f"parent_selector={variant}",
    )
    assert _exists(cfg, "parent_selector._target_")
...
def test_autoresearch_is_greedy_mutation_only_repo_evolution():
    cfg = _compose("experiment=autoresearch")

    assert cfg.num_parents == 1
    assert cfg.max_elites_per_generation == 1
    assert cfg.max_mutations_per_generation == 4
    assert cfg.island_max_size == 1
    assert cfg.primary_resolution == 1
    assert cfg.enable_migration is False
    assert cfg.repo_harness.staged_validation.enabled is False

    assert (
        cfg.parent_selector._target_
        == "gigaevo.evolution.mutation.parent_selector.RandomParentSelector"
    )
    assert cfg.parent_selector.num_parents == 1
    assert set(cfg.parent_selector) == {"_target_", "num_parents"}

    assert set(cfg.dag_blueprint.nodes) == {
        "RepoBenchmarkStage",
        "EnsureMetricsStage",
    }
    assert "RepoReflectionStage" not in cfg.dag_blueprint.nodes


def test_repo_harness_parent_portfolio_matches_mutation_budget():
    cfg = _compose(
        "experiment=repo_harness_archival_curator",
        "max_mutations_per_generation=5",
    )

    assert cfg.parent_selector.max_ranked_sets == 5
...
def test_redis_prefix_controls_current_run_namespace():
    ...
    defines `redis.prefix: ${problem.name}` in config/redis/default.yaml. A
    later regression hard-coded storage back to ${problem.name}, so different
    experiments for the same problem shared one instance lock.
    """
    cfg = _compose()
    ...
    cfg = _compose("redis.prefix=_experiment_a_")
    assert cfg.redis.prefix == "_experiment_a_"
    assert cfg.redis_storage.config.key_prefix == "_experiment_a_"
    assert cfg.redis_metrics_config.key_prefix == "_experiment_a_:metrics"

    cfg = _compose("redis.prefix=_experiment_a_", "+post_step_hook=repo_viz")
    assert cfg.post_step_hook.redis_prefix == "_experiment_a_"

    # Any pipeline that references ${redis.prefix} must now resolve cleanly.
    cfg = _compose(
        "pipeline=adversarial_coevo_ss",
        "redis.prefix=_experiment_a_",
        "opponent_redis_db=2",
        "opponent_redis_prefix=_test_opponent_",
    )
    assert cfg.pre_step_hook.own_prefix == "_experiment_a_", (
        "adversarial_coevo_ss.pre_step_hook.own_prefix must resolve to "
        "the current run namespace via ${redis.prefix}"
    )


def test_redis_selection_loader_keeps_source_prefix_separate_from_destination():
    cfg = _compose("loader=redis_selection", "redis.prefix=_destination_")
    assert cfg.redis_storage.config.key_prefix == "_destination_"
    assert cfg.program_loader.key_prefix == "_test_"

    cfg = _compose(
        "loader=redis_selection",
        "redis.prefix=_destination_",
        "loader.source_prefix=_source_",
    )
    assert cfg.redis_storage.config.key_prefix == "_destination_"
    assert cfg.program_loader.key_prefix == "_source_"
