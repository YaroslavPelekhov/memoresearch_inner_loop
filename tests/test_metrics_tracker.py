...
class TestProcessProgram:
    ...
    @pytest.mark.asyncio
    async def test_valid_program_does_not_plot_unavailable_sentinel_metric(
        self,
    ) -> None:
        ctx = _make_metrics_context(
            extra_specs={
                "core": MetricSpec(
                    description="Optional CORE score",
                    higher_is_better=True,
                    sentinel_value=-1.0,
                )
            }
        )
        prog = _make_program(metrics={VALIDITY_KEY: 1.0, "score": 10.0, "core": -1.0})
        writer = RecordingWriter()
        tracker = MetricsTracker(
            storage=_mock_storage([]), metrics_context=ctx, writer=writer
        )

        assert await tracker._process_program(prog) is True

        tags = {tag for tag, _value, _kwargs in writer.scalars}
        assert "valid/program/score" in tags
        assert "valid/program/core" not in tags
        assert "valid/frontier/core" not in tags
