...
if [[ -n "$REVIEW_FILE" ]]; then
    if grep -q '\[x\] APPROVED\|Verdict: APPROVED\|\*\*Verdict\*\*: APPROVED\|Overall Verdict: APPROVED\|## Overall.*: APPROVED' "$REVIEW_FILE"; then
...
with open('$EXP_DIR/experiment.yaml') as f:
    m = yaml.safe_load(f)
print(m.get('lifecycle',{}).get('status','') or m.get('experiment',{}).get('status',''))
