...
if [[ -f "$EXP_DIR/02_review.md" ]]; then
    if grep -q '\[x\] APPROVED\|Verdict: APPROVED\|\*\*Verdict\*\*: APPROVED\|Overall Verdict: APPROVED\|## Overall.*: APPROVED' "$EXP_DIR/02_review.md"; then
...
if [[ -f "$EXP_DIR/launch.sh" ]]; then
    ...
    fail "launch.sh missing — run: gigaevo -e $EXPERIMENT launch --dry-run"
fi
