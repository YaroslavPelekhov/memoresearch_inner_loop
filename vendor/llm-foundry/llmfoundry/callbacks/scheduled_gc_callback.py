...
class ScheduledGarbageCollector(Callback):
    ...
    def fit_start(self, state: State, logger: Logger):
        ...
        # add warmup_gc_iters to the current batch for correct gc with autoresume
        self.warmup_gc_iters += state.timestamp.batch.value
    ...
    def batch_end(self, state: State, logger: Logger):
        if state.timestamp.batch.value % self.batch_interval == 0 or \
            state.timestamp.batch.value <= self.warmup_gc_iters:
            gc_cuda()
