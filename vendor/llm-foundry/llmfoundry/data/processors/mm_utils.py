...
def get_crop_size(processor: BaseImageProcessor):
    if processor.__class__.__name__ == "CLIPImageProcessor":
        return processor.crop_size["height"]
    elif processor.__class__.__name__ == "SiglipImageProcessor":
        return processor.size["height"]
    raise ValueError(f"Unknown image_processor class: {processor.__class__.__name__}")


def process_anyres_image_onevision(image: Image, processor: BaseImageProcessor, grid_pinpoints: str):
    """
    ...
    patch_size = get_crop_size(processor)

    if isinstance(grid_pinpoints, str) and "x" in grid_pinpoints:
        OV_PATCH_SIZES = (224, 336, 384, 448, 512)
        assert patch_size in OV_PATCH_SIZES, F"patch_size should be in {OV_PATCH_SIZES}"
...
def process_anyres_image(image: Image, processor: BaseImageProcessor, grid_pinpoints: str):
    """
    Process an image with variable resolutions.
