...
from .configuration_fastvithd import FastViTHDVisionConfig
...
class FastViTHDVisionModel(PreTrainedModel):
    """
    FastViTHD Vision Model for feature extraction.

    (avtereshchenko): Original implementation was VLM-style based
    on pytorch-like FastViT (nn.Module). To create Vision Encoder,
    preserving llmf code style I manually created this PreTrainedModel-like
    model.

    Everything above FastViTHDVisionModel class is copied from original
    HF repo.
    """

    config_class = FastViTHDVisionConfig
    ...
    def __init__(self, config: FastViTHDVisionConfig):
        super().__init__(config)

        self.config = config

        for i, pos_emb in enumerate(config.pos_embs):
            if pos_emb == "none":
                config.pos_embs[i] = None
            elif pos_emb == "repcpe":
                config.pos_embs[i] = partial(RepCPE, spatial_shape=(7, 7))
            else:
                raise ValueError(f"Invalid position embedding type: {pos_emb}")

        if config.norm_layer == "normlayerchannel":
            config.norm_layer = LayerNormChannel
        else:
            raise ValueError(f"Invalid normalization layer: {config.norm_layer}")

        if config.act_layer == "gelu":
            config.act_layer = nn.GELU
        else:
            raise ValueError(f"Invalid activation layer: {config.act_layer}")
        ...
        self.patch_embed = convolutional_stem(
            ...
            use_scale_branch=config.stem_scale_branch,
        )
        ...
        for i in range(len(config.layers)):
            ...
            if config.pos_embs[i] is not None:
                network.append(
                    config.pos_embs[i](
                        config.embed_dims[i],
                        config.embed_dims[i],
            ...
            if (
                ...
                or config.embed_dims[i] != config.embed_dims[i + 1]
            ):
                network.append(
                    PatchEmbed(
                        ...
                        inference_mode=config.inference_mode,
                        use_se=config.se_downsamples[i + 1],
                    )
                )

        self.network = nn.ModuleList(network)

        # Classifier head (conv_exp layer for feature extraction)
    ...
    def forward_tokens(
        ...
        """Forward through network tokens."""
        # (avtereshchenko): output_hidden_states is not used in original code
    ...
    def forward_encoder(
        ...
        """
        Analogue of InternViT self.encoder.
        """
        ...
        for _, encoder_layer in enumerate(self.network):
            if output_hidden_states:
                encoder_states = encoder_states + (hidden_states,)
        ...
        hidden_states = self.conv_exp(hidden_states)
    ...
    def forward(
        ...
        # (avtereshchenko): output_hidden_states and return_dict are not used in original code.
        # Code is aligned with InternViT forward method for easy support and development.
        output_hidden_states = (
            output_hidden_states
            if output_hidden_states is not None
            else self.config.output_hidden_states
