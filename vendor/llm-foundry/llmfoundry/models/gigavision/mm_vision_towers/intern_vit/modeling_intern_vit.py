...
#
# NOTE (Sbr): in comparison to the original modelling file,
# this one does not try to use FusedRMSNorm from
# apex.normalization (if apex is installed) instead of
# InternRMSNorm and has gradient_checkpointing diabled.
# --------------------------------------------------------
...
class InternVisionEmbeddings(nn.Module):
    ...
    def _get_pos_embed(self, pos_embed, H, W):
        ...
        if (
            ...
            or W != self.image_size // self.patch_size
        ):
...
class InternAttention(nn.Module):
    ...
    def __init__(self, config: InternVisionConfig):
        ...
        self.use_flash_attn = config.use_flash_attn and _has_flash_attn
        if (
            config.use_flash_attn
            and not _has_flash_attn
            and torch.distributed.get_rank() == 0
        ):
            logger.info(
                "Flash Attention is not available and will not be used in vision model's attention, "
                "use_flash_attn is set to False."
            )
        ...
        if self.use_flash_attn:
            self.inner_attn = FlashAttention(
                ...
                deterministic=self.config.deterministic_attention,
            )
...
class InternVisionEncoder(nn.Module):
    ...
    def __init__(self, config: InternVisionConfig):
        ...
        # NOTE (Sbr): Disable gradient checkpointing here, it doesn't work with gigafsdp
...
class InternVisionModel(PreTrainedModel):
    ...
    _no_split_modules = ["InternVisionEncoderLayer", "InternVisionEmbeddings"]
