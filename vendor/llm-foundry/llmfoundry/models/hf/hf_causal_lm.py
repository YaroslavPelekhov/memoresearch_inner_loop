    ...
    def __init__(self, om_model_config: Union[DictConfig,
        ...
        if isinstance(om_model_config, DictConfig):
            ...
            if resolved_init_device == "cpu":
                ...
                if om_model_config.pretrained:
                    raise ValueError('Setting cfg.pretrained=True is not supported when init_device="meta".')
                with init_empty_weights(include_buffers=False):
                    model = AutoModelForCausalLM.from_config(
                        config,
                        trust_remote_code=trust_remote_code,
                ...
                raise ValueError(f'init_device="{init_device}" must be either "cpu" or "meta".')

            # signal_file_path = f'.node_{dist.get_node_rank()}_local_rank0_completed'
            signal_file_path = f'/tmp/.node_{dist.get_node_rank()}_local_rank0_completed'
