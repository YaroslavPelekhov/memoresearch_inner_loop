...
def _get_model_forward_args(forward) -> list[str]:
    return [
        param.name
        for param in inspect.signature(forward).parameters.values()
...
class HuggingFaceModelWithZLoss(HuggingFaceModel):
    ...
    def __init__(self,
        ...
        self.model_forward_args = _get_model_forward_args(self.model.forward)
