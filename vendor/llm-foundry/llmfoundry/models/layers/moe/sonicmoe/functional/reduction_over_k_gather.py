...
def token_gather_sum_kernel(
    ...
    # tile sizes (config params, injected by autotuner - must be last)
    ...
    if is_varlen_K:
        ...
        K_this_token = Me - Ms  # actual K for this token (uint32)
        n_k_tiles = (K_this_token.to(tl.int32) + BLOCK_K - 1) // BLOCK_K  # int32 // int32
    else:
        Ms = MAX_K * t_idx
        K_this_token: tl.constexpr = MAX_K
        n_k_tiles: tl.constexpr = (MAX_K + BLOCK_K - 1) // BLOCK_K  # Python int
