...
from llmfoundry.models.ops.float8.triton_kernels.block_quantization import block_quantization_128x128_fn
...
def test_block_quantization(num_groups: int, int_dim: int, hid_dim: int) -> None:
    ...
    for group in range(num_groups):
        ...
        wq_te_bfp16 = wq_te._rowwise_data.view(torch.float8_e4m3fn).to(torch.bfloat16)

        wq_triton = block_quantization_128x128_fn(weights[group].unsqueeze(0))
