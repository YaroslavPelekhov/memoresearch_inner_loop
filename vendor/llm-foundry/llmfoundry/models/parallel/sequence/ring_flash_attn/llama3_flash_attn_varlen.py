...
def llama3_flash_attn_varlen_backward(
    ...
    for i in range(0, nheads_k, heads_k_stride):
        ...
        handle = dist.reduce_scatter_tensor(dk_i, dkv_buffer[0], group=process_group, async_op=True)
        handle.wait()
        handle = dist.reduce_scatter_tensor(dv_i, dkv_buffer[1], group=process_group, async_op=True)
        handle.wait()
