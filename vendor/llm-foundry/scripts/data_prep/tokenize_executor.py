...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to a tokenizer to use for data tokenization.",
    )
    parser.add_argument(
        "--max-tokenized-len",
        type=int,
        required=True,
        help="A maximum number of tokens to use for a sample.",
    )
    parser.add_argument(
...
def process_dataset(process_path_tuple: tuple, script: str, script_kwargs: Dict[str, str]):
    """
...
def main(args):
    ...
    args:
        ...
        - `tokenizer_path` -- path to a tokenizer used for data processing,
        - `max_tokenized_len` -- maximum tokenization length,
    ...
    concat_tokens = args.max_tokenized_len
    ...
    assert os.path.exists(task_file), f"Provided tasks file does not exist: {task_file}!"
    ...
    if script_additional_params != "":
        ...
        script_additional_params = [param.split("=") for param in script_additional_params]
        for param_name, param_val in script_additional_params:
            script_kwargs[param_name] = param_val

    process_dataset_updated = partial(process_dataset, script=script, script_kwargs=script_kwargs)
    ...
    executor_tasks = [
        ...
        if (
            ...
            or (Path(task[1]).is_dir() and not (Path(task[1]) / "index.json").exists())
        )
    ]
    for task in executor_tasks:
    ...
    print(f"Executor tasks to process: {executor_tasks}")
